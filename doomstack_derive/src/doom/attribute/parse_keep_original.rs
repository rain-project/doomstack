...
impl Attribute {
    ...
    ///
    /// Inputs the (optional) `body` of a `#[doom(keep_original)]` or `#[doom(keep_original(body))]`
    /// attribute, as parsed by [`Attribute::parse_parts`]. Expects `body` (if it exists) to be a
    /// boolean expression representing the condition to `keep_original`. Returns a
    /// [`KeepOriginal`].
